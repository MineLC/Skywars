...
import lc.mine.skywars.game.kit.gui.KitInventory;
...
public final class KitConfigLoader {
    ...
    public KitConfigLoader(Logger logger) {
        this.logger = logger;
    }

    public void load(final SkywarsPlugin plugin, final Yaml yaml, final String defaultKitName, final KitConfig kitsConfig) {
        final File[] kitsFiles = getKitsFiles(plugin);
        ...
        kitsConfig.setArrayKits(kits);
        kitsConfig.setKitsPerName(perName);
        kitsConfig.setDefaultKit(defaultKit);
        kitsConfig.setInventory(inventory);
    }

    private File[] getKitsFiles(final SkywarsPlugin plugin) {
